...
DATE = "2011_09_26"
SPLIT = "2011_09_26_drive_0005_sync"
FRAMES = range(113, 124)
P = 10
O = 0.3
...
df_ = df_grouped.get_group("110926_0005_4")
# for _, row in df_110926_0014_2.iterrows():
df_.set_index("imageIndex", inplace=True)
a = df_.filter(["distance", "overlap", "diff_u0", "diff_u1", "diff_v0", "diff_v1"])
for frame in FRAMES:
    row = df_.loc[frame]
    ...
    # plt.show()
    plt.axis("off")
    plt.savefig(f"110926_0005_4_{frame}_p{P}_o{O}_boxes.png", bbox_inches="tight", dpi=1000)
    plt.cla()
print("The End.")
